...
Partial Public Class PostGLViewController
    ...
    Private Sub PostGL_Execute(sender As Object, e As SimpleActionExecuteEventArgs) Handles PostGL.Execute
        Dim os As IObjectSpace = Me.Application.CreateObjectSpace()
        Try
            For Each BookRVinfo As AccountBookRV In e.SelectedObjects
                For i As Integer = 0 To BookRVinfo.AccountID.Count - 1
                    'อันดับแรกเช็คก่อนว่า รหัสบัญชีนี้อยู่หมวดใหน
                    Dim StrLeft As String = Left(BookRVinfo.AccountID(i).Account.AccountID, 2)
                    ...
                    If ChkGL Is Nothing Then
                        ...
                        If LoadBringforward IsNot Nothing Then
                            ...
                            Transaction.DocuNo = ""
                            Transaction.AccountBook = BookRVinfo.AccountBookRV.AccountBookNameVGA
                            Transaction.IVRefNo = ""
                            Transaction.RefNo = ""
                            Transaction.RVDesc1 = ""
                        ...
                        ADDGL.DocuNo = BookRVinfo.DocumentNoRV
                        ADDGL.DocuDate = BookRVinfo.DateListRV
                        ADDGL.AccountBook = BookRVinfo.AccountBookRV.AccountBookNameVGA
                        ADDGL.IVRefNo = BookRVinfo.ReceiptNoRV
                        ADDGL.RefNo = BookRVinfo.ReferenceNoRV
                        ADDGL.RefDate = BookRVinfo.DateDateRV
                        ADDGL.RefAmnt = BookRVinfo.AmountRV
                        ADDGL.ListNo = BookRVinfo.ListNoRV
                        ADDGL.RVDesc1 = BookRVinfo.DetailRV
                        ADDGL.ToptotalAmnt = BookRVinfo.MoneyRV
                        ADDGL.AccID = BookRVinfo.AccountID(i).Account.AccountID
                        ADDGL.AccountName = BookRVinfo.AccountID(i).Account.AccountName

                        Select Case AccountInfo.AccountName
                            Case "สินทรัพย์" 'เพิ่มอยู่ฝั่ง Debit
                                AccountTypeEnum = EnumAccountType.Debit
                                ADDGL.TotalAmnt = LoadBringforward.Debit + BookRVinfo.AccountID(i).Debit
                            Case "หนิ้สิน" 'เพิ่มอยู่ฝั่ง Credit
                                AccountTypeEnum = EnumAccountType.Credit
                                ADDGL.TotalAmnt = LoadBringforward.Crebit + BookRVinfo.AccountID(i).Credit
                            Case "ทุน" 'เพิ่มอยู่ฝั่ง Credit
                                AccountTypeEnum = EnumAccountType.Credit
                                ADDGL.TotalAmnt = LoadBringforward.Crebit + BookRVinfo.AccountID(i).Credit
                            Case "รายได้" 'เพิ่มอยู่ฝั่ง Credit
                        ...
                        ADDGL.AccDetail = BookRVinfo.DetailRV
                        ADDGL.DrAmnt = BookRVinfo.AccountID(i).Debit
                        ADDGL.CrAmnt = BookRVinfo.AccountID(i).Credit
                        ADDGL.Save()
                    ...
                    Else
                        ...
                        Dim AccountCollection = ObjectSpace.CreateCollection(GetType(GLID), DevExpress.Data.Filtering.ContainsOperator.Parse("AccID=?", BookRVinfo.AccountID(i).Account.AccountID))
                        ...
                        Select Case AccountInfo.AccountName
                            Case "สินทรัพย์" 'เพิ่มอยู่ฝั่ง of
                                AccountTypeEnum = EnumAccountType.Debit
                                ADDGL.TotalAmnt = GetAmout + BookRVinfo.AccountID(i).Debit
                            Case "หนิ้สิน" 'เพิ่มอยู่ฝั่ง Credit
                                AccountTypeEnum = EnumAccountType.Credit
                                ADDGL.TotalAmnt = GetAmout + BookRVinfo.AccountID(i).Credit
                            Case "ทุน" 'เพิ่มอยู่ฝั่ง Credit
                                AccountTypeEnum = EnumAccountType.Credit
                                ADDGL.TotalAmnt = GetAmout + BookRVinfo.AccountID(i).Credit
                            Case "รายได้" 'เพิ่มอยู่ฝั่ง Credit
                            ...
                            Case "ค่าใช้จ่าย" 'เพิ่มอยู่ฝั่ง Debit
                                AccountTypeEnum = EnumAccountType.Debit
                                ADDGL.TotalAmnt = GetAmout + BookRVinfo.AccountID(i).Debit 'BookRVinfo.AccountID(i - 1).Debit + BookRVinfo.AccountID(i).Debit
                        End Select
